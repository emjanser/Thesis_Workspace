...
for i in range(1,3):
    ...
    plt.legend()
    filename = f'loss_{im}_{seed_number}.png'
    plt.savefig(filename, dpi = 300)
    
    model.save(f'ann_model_{im}_{seed_number}.h5')
...
for i in range(1,3):
    seed_number = int(i*10)
    loaded_model = tf.keras.models.load_model(f'ann_model_{im}_{seed_number}.h5')
        
    data_path = '../airfoil_shapes_re_v2/'   
    
    db = abs_path + data_path + 'test_data.csv'
